...
describe('selectors in attributes', () => {
  it('case 1', () => {
    const container = document.createElement('div');

    const tpl = () => html`
      <input target='[ref="slider"]' />
    `;

    render(tpl(), container);
    expect(container.querySelector('input').getAttribute('target')).toBe('[ref="slider"]');
  });
  ...
  it('case 2', () => {
    ...
    const tpl = () => html`
      <input container="[id='tooltip-container']" />
    `;

    render(tpl(), container);
    expect(container.querySelector('input').getAttribute('container')).toBe("[id='tooltip-container']");
  });
});
